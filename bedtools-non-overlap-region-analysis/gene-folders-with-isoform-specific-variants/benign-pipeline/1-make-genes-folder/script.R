...
# Read the BED file
bed_data <- read.table("variants-sorted-benign.bed", header = FALSE, stringsAsFactors = FALSE, fill=TRUE)

# Read the gene names from the CSV file
gene_names <- read.csv("gene_names.csv", header = FALSE, stringsAsFactors = FALSE)[, 1]

# Create the superfolder "benign-genes"
superfolder <- "benign-genes"
...
cat("All processing complete. Filtered data saved in the 'benign-genes' superfolder.\n")
