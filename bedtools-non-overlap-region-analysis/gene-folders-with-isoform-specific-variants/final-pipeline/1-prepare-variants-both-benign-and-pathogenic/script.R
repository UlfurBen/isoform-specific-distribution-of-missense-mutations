...
output_file <- "variants_benign_pathogenic_non_vus_non_conflicting.bed"
...
# Define the inclusion terms
inclusion_terms <- c("benign", "pathogenic")
...
filtered_data <- bed_data %>%
  filter(grepl(paste(inclusion_terms, collapse = "|"), V10, ignore.case = TRUE) & 
