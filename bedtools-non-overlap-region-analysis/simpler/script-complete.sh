...
# Remove all rows with first column value that isn't numeric

# Load necessary library
library(dplyr)

# Function to check if a value is numeric without coercion
is_strictly_numeric <- function(x) {
  grepl("^\\d+$", x)
}
...
# Filter rows where the first column is strictly numeric
filtered_data <- bed_data %>%
  filter(sapply(V1, is_strictly_numeric))

# Write the filtered data back to a new BED file
write.table(filtered_data, "filtered_bed_file.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)

print("Filtering complete. The new file is saved as 'filtered_bed_file.bed'.")




# Sort by chr and chromStart
...
# Convert first two columns to numeric for sorting
