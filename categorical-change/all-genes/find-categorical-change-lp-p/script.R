# Define file paths
# Get input_file by running grep -i 'pathogenic' homo_sapiens_variation_missense_ClinVar.txt | grep -i 'likely pathogenic' | grep -vi 'uncertain' | grep -vi 'benign' | grep -vi 'likely benign' > homo_sapiens_variation_missense_ClinVar_pathogenic_likely_pathogenic_only_all_genes.txt
input_file <- "homo_sapiens_variation_missense_ClinVar_pathogenic_likely_pathogenic_only_all_genes.txt"
property_lookup_file <- "Amino_acid_change_with_property_change.txt"
output_file <- "homo_sapiens_variation_missense_ClinVar_pathogenic_likely_pathogenic_only_all_genes_aa_change.txt"
...
colnames(data) <- c("Column1", "Isoform", "AminoAcidChange", "Column4", "Column5", "Column6", "Column7", "Column8", "Column9", "Column10", "Column11", "Column12", "Column13", "Column14")
