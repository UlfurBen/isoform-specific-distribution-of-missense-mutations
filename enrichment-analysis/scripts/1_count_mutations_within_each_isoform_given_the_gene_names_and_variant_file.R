...
count_variants <- function(identifier, variant_type, exclusion_patterns, exclude_hyphen = FALSE) {
  ...
  print(paste("Count for", identifier, variant_type, ":", count))  # Debug print statement
  return(count)
}
...
for (gene_name_human in temp_gene_names) {
  ...
  # Verify the content of temp.txt
  ...
  for (identifier in canonical_identifiers) {
    ...
    for (variant_type in names(exclusion_patterns)) {
      count_mutation <- count_variants(identifier, variant_type, exclusion_patterns[[variant_type]], exclude_hyphen = TRUE)
  ...
  for (identifier in temp_identifiers_with_hyphen) {
    variant_counts <- c()
    for (variant_type in names(exclusion_patterns)) {
      count_mutation <- count_variants(identifier, variant_type, exclusion_patterns[[variant_type]], exclude_hyphen = FALSE)
      variant_counts <- c(variant_counts, count_mutation)
    }
    safe_write(paste0(identifier, ",", paste(variant_counts, collapse = ",")))
  }
