...
output_file <- "calculate_pathogenic_variants_per_isoform.txt"
...
for (gene_name_human in temp_gene_names) {
  ...
  for (identifier in temp_identifiers_2) {
    count_mutation <- as.numeric(system(paste0("grep -w '", identifier, "' ", variation_file, " | grep 'missense variant' | grep 'Pathogenic' | grep -v 'Likely pathogenic' | grep -v 'pathogenicity' | grep -v 'uncertain' | grep -v 'benign' | wc -l"), intern = TRUE))
    if (count_mutation > 0) {
      safe_write(paste0(identifier, ",", count_mutation))
    }
  }
