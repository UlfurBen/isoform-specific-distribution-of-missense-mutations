...
output_file <- "count_ranges_with_zero_variant_count.txt"
...
range_counts["0"] <- sum(data$Count == 0, na.rm = TRUE)
range_counts["0-5"] <- sum(data$Count >= 1 & data$Count <= 5, na.rm = TRUE)
